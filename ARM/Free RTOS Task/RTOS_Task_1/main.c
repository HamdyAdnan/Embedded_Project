/**
 ******************************************************************************
 * @file           : main.c
 * @author         : Auto-generated by STM32CubeIDE
 * @brief          : Main program body
 ******************************************************************************
 * @attention
 *
 * Copyright (c) 2023 STMicroelectronics.
 * All rights reserved.
 *
 * This software is licensed under terms that can be found in the LICENSE file
 * in the root directory of this software component.
 * If no LICENSE file comes with this software, it is provided AS-IS.
 *
 ******************************************************************************
 */

//#include <stdint.h>
//
//#if !defined(__SOFT_FP__) && defined(__ARM_FP)
//  #warning "FPU is not initialized, but the project is compiling for an FPU. Please initialize the FPU before use."
//#endif
#include "_delay_ms.h"
#include "STD_TYPES.h"
#include "RCC_init.h"
#include "GPIO_init.h"
#include "FreeRTOS.h"
#include "task.h"

TaskHandle_t	xRedLedTaskHandle;
TaskHandle_t	xGreenLedTaskHandle;

void	vRedLedTask(void *pvParameter);
void	vGreenLedTask(void *pvParameter);
int main(void)
{
	M_RCC_void_SysClkInit();
	M_RCC_void_EnablePerClk(RCC_APB2, 2);
	M_GPIO_void_SetPinDir(PORTA_ID, PIN0_ID, OUT_50MHZ_PP);
	M_GPIO_void_SetPinDir(PORTA_ID, PIN1_ID, OUT_50MHZ_PP);

	   xTaskCreate(     vRedLedTask , "RED TASK LED", 128, NULL , 1 , &xRedLedTaskHandle);
	   xTaskCreate(     vGreenLedTask , "GREEN TASK LED", 128, NULL , 1 , &xGreenLedTaskHandle);
	   vTaskStartScheduler();
    /* Loop forever */
	for(;;);
}

void	vRedLedTask(void *pvParameter)
{
	for(;;)
	{
		M_GPIO_void_SetPinValue(PORTA_ID, PIN0_ID, HIGH);
		//_delay_ms(1000);
		vTaskDelay(1000);
		M_GPIO_void_SetPinValue(PORTA_ID, PIN0_ID, LOW);
		vTaskDelay(1000);
		//_delay_ms(1000);
	}
}
void	vGreenLedTask(void *pvParameter)
{
	for(;;)
	{
		M_GPIO_void_SetPinValue(PORTA_ID, PIN1_ID, HIGH);
		//_delay_ms(1000);
		vTaskDelay(1000);
		M_GPIO_void_SetPinValue(PORTA_ID, PIN1_ID, LOW);
		vTaskDelay(1000);
		//_delay_ms(1000);
	}
}
